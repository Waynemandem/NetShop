...
const DB_VERSION = 3; // Incremented version to force upgrade
const STORES = {
    ...
    static async init() {
        ...
        return new Promise((resolve, reject) => {
            ...
            request.onupgradeneeded = (event) => {
                ...
                Object.values(STORES).forEach(storeName => {
                    if (!db.objectStoreNames.contains(storeName)) {
                        // All stores use 'id' as keyPath except IMAGES which might use productId as key
                        // But for simplicity let's assume 'id' for all structured data, 
                        // and IMAGES might be different.
                        // The original code had specific checks. Let's replicate that logic but cleaner.
                        if (storeName === STORES.IMAGES) {
                            db.createObjectStore(storeName); // No keyPath, key is passed in put()
                        } else {
                            db.createObjectStore(storeName, { keyPath: 'id' });
                        }
                    }
